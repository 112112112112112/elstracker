...
export default function FragmentTracker({ characters }) {
    ...
    useEffect(() => {
        async function load() {
            const data = await window.db.getAllFragments();
    ...
    const handleUpdate = async (characterId, fragmentType, amount) => {
        await window.db.updateFragments(characterId, fragmentType, amount);
        const data = await window.db.getAllFragments();
        setFragmentData(data);
    };

    const handleSet = async (characterId, fragmentType, value) => {
        await window.db.setFragments(characterId, fragmentType, value);
        const data = await window.db.getAllFragments();
        setFragmentData(data);
    };

    const handleReset = async (characterId, fragmentType) => {
        await window.db.resetFragments(characterId, fragmentType);
        const data = await window.db.getAllFragments();
        setFragmentData(data);
    };
