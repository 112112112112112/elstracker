...
const tasks = [
    ...
    { title: `Mirror Del`, reset: `weekly`, bound: `character`},
    { title: `Devil of Chaos`, reset: `weekly`, bound: `character`},
    { title: `High Entropy`, reset: `weekly`, bound: `character`},
];
...
for (const task of tasks) {
    ...
    if (!taskExists) {
        db.prepare('INSERT INTO tasks(title, reset, bound, system) VALUES (?, ?, ?, ?)').run(task.title, task.reset, task.bound, 1);
    }
}
